...
# Make win64
export QSS_CFLAGS="-DQSS_REVISION=`git rev-parse HEAD` -DQSS_DATE=`git log -1 --date=short --format=%cd`"
export QSS_LDFLAGS=""
...
# Make Linux64
export QSS_CFLAGS="-DQSS_VERSION=`git describe --always`-DQSS_REVISION=`git rev-parse HEAD` -DQSS_DATE=`git log -1 --date=short --format=%cd`"
export QSS_LDFLAGS=""
make clean
make $MAKEARGS
