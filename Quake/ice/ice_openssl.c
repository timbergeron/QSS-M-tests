...
{
	icestream_t *f = BIO_get_data(h);
	int r = f->WriteBytes(f, buf, size);
...
static long OSSL_Bio_FCtrl(BIO *h, int cmd, long arg1, void *arg2)
{
	...
	{
	case BIO_CTRL_FLUSH:
		f->ReadBytes(f, NULL, 0);	//flushes as a side effect.
		return 1;
	default:
...
static int OSSL_FRead (icestream_t *file, void *buffer, int bytestoread)
{
...
static int OSSL_FWrite (icestream_t *file, const void *buffer, int bytestowrite)
{
	osslvfs_t *o = (osslvfs_t*)file;
...
static qboolean OSSL_Close (icestream_t *file)
{
	osslvfs_t *o = (osslvfs_t*)file;
	BIO_free(o->bio);
	SSL_CTX_free(o->ctx);
...
static qboolean OSSL_SetCredentials(struct dtlslocalcred_s *cred)
{
	const unsigned char *cert = cred->cert;
	const unsigned char *key = cred->key;

	X509_free(vhost.servercert);
	EVP_PKEY_free(vhost.privatekey);

	vhost.privatekey = d2i_AutoPrivateKey(NULL, &key, cred->keysize);
	vhost.servercert = d2i_X509(NULL, &cert, cred->certsize);

	//FIXME: come up with some way to verify that the key matches the cert...
	return vhost.privatekey && vhost.servercert;
}
...
static icestream_t *OSSL_OpenVFS(const char *hostname, icestream_t *source, qboolean isserver)
{
	BIO *sink;
	osslvfs_t *n;
	if (!OSSL_Init())
...
static int OSSL_Bio_DRead(BIO *h, char *buf, int size)
{
	...
	if (f->pending)
	{
		if (size > f->pendingsize)
			size = f->pendingsize;
		memcpy(buf, f->pending, size);
...
static unsigned int OSSL_CL_Validate_PSK(SSL *ssl, const char *hint, char *identity, unsigned int max_identity_len, unsigned char *psk, unsigned int max_psk_len)
...
static neterr_t OSSL_Received(void *ctx, netadr_t *from, const void *src_data, size_t src_size, void *out_data, size_t out_maxsize, size_t *out_used)
{	//we have received some encrypted data...
	ossldtls_t *o = (ossldtls_t*)ctx;
	int r;

	o->pending = src_data;
	o->pendingsize = src_size;
	r = BIO_read(o->bio, out_data, out_maxsize);
	o->pending = NULL;
	o->pendingsize = 0;
...
{	//keep it ticking over, or something.
	return OSSL_Received(ctx, NULL, NULL, 0, NULL, 0, NULL);
}
...
static qboolean OSSL_GenTempCertificate(const char *subject, struct dtlslocalcred_s *cred)
{
	...
	cred->key = ffs = malloc(cred->keysize);
...

	{
		...
		cred->cert = ffs = malloc(cred->certsize);
...
//		BIO_meth_set_gets(biometh_dtls, OSSL_Bio_DGets);
		...
		Sys_RandomBytes(dtlscookiekey, sizeof(dtlscookiekey));	//something random so people can't guess cookies for arbitrary victim IPs.
	}
...
/*static void OSSL_Shutdown(void)
{
	ossl.inited = false;
