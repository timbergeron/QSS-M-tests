(function () {
  ...
  class QuakeWebChatClient {
    ...
    bindUi() {
      ...
      this.ui.clearButton.addEventListener("click", () => {
        this.ui.consoleLog.textContent = "";
      });
      ...
      if (this.ui.debugToggle) {
        this.ui.debugToggle.addEventListener("change", () => {
          ...
          this.logLocal(`Verbose debug ${this.ui.debugToggle.checked ? "enabled" : "disabled"}`);
        });
      }
    ...
    connect() {
      ...
      this.logLocal(`Opening ${url} as quake/fteqw WebSocket`);
    ...
    onSocketOpen() {
      this.logLocal("WebSocket open, sending Quake connect request");
    ...
    onSocketMessage(event) {
      if (!(event.data instanceof ArrayBuffer)) {
        this.logLocal("Ignoring non-binary WebSocket message");
        return;
      }
    ...
    handleControlPacket(bytes) {
      ...
      if (reply === CCREP_ACCEPT) {
        ...
        this.logLocal(`Server ${acceptNote}`);
    ...
    handleGameMessage(bytes) {
      ...
      while (!reader.eof()) {
        ...
        if (opcode & 0x80) {
          this.logLocal(`Fast entity update not parsed in text client (opcode 0x${opcode.toString(16)})`);
          return;
        }
    ...
    parseServerOpcode(reader, opcode) {
      switch (opcode) {
        ...
        case svc_temp_entity:
          this.logLocal("Temp entity packet skipped");
          return false;
        ...
        case svc_centerprint:
          this.logServer(`[center] ${reader.readString()}`);
          return true;
        ...
        case svc_skybox:
          this.logLocal(`Skybox: ${reader.readString()}`);
          return true;
    ...
    handleStuffText(text) {
      ...
      if (trimmed === "cmd pext") {
        this.logLocal("Server requested extension probe, replying without optional FTE extensions");
        this.sendCommand("pext 0 0 0 0\n", { raw: true });
        return;
      }

      if (trimmed === "cmd protocols") {
        this.logLocal("Server requested protocol list");
        this.sendCommand("protocols 999 666 10002 15\n", { raw: true });
        return;
      }

      this.logTrace(`stufftext ${truncateLogText(trimmed, 140)}`);
      this.logServer(`[stuff] ${text}`);
    }

    handleSignon(stage) {
      if (stage <= this.signon) {
        this.logLocal(`Ignoring stale signon ${stage}`);
        return;
      }
      ...
      if (stage === 1) {
        this.logLocal(`Signon ${stage}: sending initial userinfo, name, color, and prespawn`);
      ...
      if (stage === 2) {
        this.logLocal(`Signon ${stage}: sending spawn`);
        this.sendCommand("spawn ", { raw: true });
        return;
      }

      if (stage === 3) {
        this.logLocal(`Signon ${stage}: sending begin`);
        this.sendCommand("begin", { raw: true });
        this.updateInputState();
        return;
      }

      this.logLocal(`Signon ${stage}`);
    }
    ...
    skipSoundPacket(reader) {
      ...
      if (fieldMask & SND_FTE_MOREFLAGS) {
        this.logLocal("Sound extension flags not supported in web chat, dropping rest of packet");
    ...
    renderPlayers() {
      ...
      for (const player of rows) {
        ...
        if (player.name.toLowerCase() === this.connectedName.toLowerCase()) {
          row.classList.add("self");
        }

        row.innerHTML = `
          <td class="slot">${player.slot}</td>
    ...
    sendMove() {
      this.sendUnreliable(this.makeMoveCommand(1, 0));
    ...
    wireDrain() {
      ...
      if (this.wireQueue.length === 0) return;
    ...
    async copyConsoleLog() {
      const text = this.ui.consoleLog.textContent || "";
      if (!text.trim()) {
    ...
    logTrace(text) {
      if (this.ui.debugToggle && this.ui.debugToggle.checked) {
        this.appendLog(`[trace] ${text}`);
      }
    }
    ...
    appendLog(text) {
      ...
      this.ui.consoleLog.textContent += `${line}\n`;
  ...
  function readDebugPreference() {
    ...
    try {
      return window.localStorage.getItem("qssm-webchat-debug") === "1";
    } catch (error) {
      return false;
    }
  }
  ...
  function writeDebugPreference(enabled) {
    try {
      window.localStorage.setItem("qssm-webchat-debug", enabled ? "1" : "0");
    } catch (error) {
